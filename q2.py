...
class SKLearnDecisionTree:
    ...
    def train_model(self, criterion="entropy"):
        ...
        ### Student Code start 
        self.model = DecisionTreeClassifier(random_state = 42,criterion=criterion)
        
        self.tree = self.model.fit(self.X_train, self.Y_train)
        Y_train_pred = self.model.predict(self.X_train)
        Y_test_pred = self.model.predict(self.X_test)

        # Calculate accuracy for training and testing data
        train_accuracy = metrics.accuracy_score(self.Y_train, Y_train_pred)
        test_accuracy = metrics.accuracy_score(self.Y_test, Y_test_pred)
    ...
    def train_model_regularised(self, criterion="entropy"):
        ...
        for depth in range(1,11):
            for split in range(3,31,3):
                for leaf in range(1,11):
                    model = DecisionTreeClassifier(random_state = 42,criterion=criterion, max_depth=depth,
