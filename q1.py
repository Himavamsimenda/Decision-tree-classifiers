...
def gini_index(classes):
    ...
    gini_impurity_value = np.sum((probabilities)**2)
    gini_impurity_value = 1 - gini_impurity_value
...
def information_gain(examples, classes, attr):
    ...
    entropies_after_split = []
    for value, count in zip(attr_values, attr_counts):
        subset_indices = np.where(examples[:, attr] == value)
        subset_classes = classes[subset_indices]
        entropy_after_split = entropy(subset_classes)
        entropies_after_split.append((count / len(classes)) * entropy_after_split)
    
    # Duplicate the classes vertically, to match each example
    # Calculate the weighted sum of entropies for each attribute value
    weighted_entropy_after_split = np.sum(entropies_after_split)
...
def gini_gain(example, classes, attr):
    ...
    gini_before_split = gini_index(classes)
    ## Calculate the gini index after splitting for all unique values of the attribute
    gini_after_split = []
    for value, count in zip(attr_values, attr_counts):
        subset_indices = np.where(example[:, attr] == value)
        subset_classes = classes[subset_indices]
        gini_after = gini_index(subset_classes)
        gini_after_split.append((count / len(classes)) * gini_after)
    # For each unique value of the attribute, calculate the gini index of the split, and add it to the weighted sum
    weighted_gini_after_split = np.sum(gini_after_split)
    gini_gain_value = gini_before_split - weighted_gini_after_split
    ...
    def __build_tree__(self, features, classes, attributes, depth=0):
        ...
        if (len(attributes) == 0):
            ...
            for attr in attributes:
                infogain = information_gain(features,classes,attr)
            ...
            for value in unique_values:
               subset_indices = np.where(features[:, best_attr] == value)
